const prices = {
  milk: { migros: 2.3, coop: 1.85, lidl: 1.39, aldi: 1.59 },
  bread: { migros: 1.0, coop: 1.0, lidl: 0.99, aldi: 0.99 },
  eggs: { migros: 4.25, coop: 4.1, lidl: 4.79, aldi: 4.19 },
};
...
async function milkHandler() {
  const response = await fetch("/api/select-milk");
...
async function breadHandler() {
  const response = await fetch("/api/select-bread");
...
async function eggsHandler() {
  const response = await fetch("/api/select-eggs");
